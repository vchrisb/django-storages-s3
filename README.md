...
| `public_domain` | `AWS_S3_PUBLIC_DOMAIN` | `None` | Public domain for generated bucket-path URLs when `endpoint_url` is only reachable by Django. Must not end in `/`. If `querystring_auth` is enabled, presigned URLs are generated against this domain. |
