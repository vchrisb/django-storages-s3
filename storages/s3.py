...
def _filter_download_params(params):
    return {
        key: value for (key, value) in params.items() if key in ALLOWED_DOWNLOAD_ARGS
    }
...
class S3Storage(CompressStorageMixin, BaseStorage):
    ...
    def unsigned_connection(self):
        ...
        if unsigned_connection is None:
            session = self._create_session()
            config = self.client_config.merge(Config(signature_version=UNSIGNED))
    ...
    def _normalize_name(self, name):
        ...
        try:
            ...
            raise SuspiciousOperation(f"Attempted access to '{name}' denied.") from exc
